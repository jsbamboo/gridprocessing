...
readonly MACHINE="tuo"
readonly COMPILER="crayamd-mphipcc"
#readonly MACHINE="dane"
#readonly COMPILER="oneapi-ifx"
readonly CHECKOUT="ChileRRMxx_tuo0521"
readonly BRANCH="jzhang/ChileRRMxx_tuo0521"
readonly CHERRY=( )
...
readonly NUDGING_ROOT="/p/vast1/zhang73/nudging.UVTQ/L128.TRaave_Chilene32x32v1pg2.UVTQ.0.25plev.v1" #ERA5pres_NERSC m3522

# Directory where your code is
readonly CODE_ROOT="/p/vast1/zhang73/GitTmp/${CHECKOUT}"
...
readonly CASE_ROOT="/p/vast1/zhang73/E3SM_simulations/ChileRRMxx/${CASE_NAME}"
...
readonly run="64x1_nmonthsx1_UVTQ1d-elmr-s20250101-O38-notms-wgt1"
#readonly run="32x1_nhoursx1_UVTQ1d-elmr-s20250101-O38-notms-wgt1"
  ...
  units=${tmp[1]%%x*}
  length=${tmp[1]##*x}
  walltime="4:30:00" #32 1mon
  if [ "$units" == "nhours" ]; then
    walltime="00:59:00" #O31
  fi
...
cat <<EOF >> user_nl_elm
 ...
 finidat = '/p/vast1/zhang73/grids2/Chilene32x32v1/gridprocessing/elm.r.2025-01-01-00000.IELM.ChileRRMxx.dane.interpinic_datm3h.nc'
...
cat > user_nl_cice << 'eof'
 stream_fldfilename = '/p/vast1/zhang73/DATA/data_hiccup/sst_ice.daymean.2020_2025_noleap.fillmsg.aave_1x1.fmt-c260502.nc'
...
case_setup() {
    ...
    if [ "$nnodes" -le 16 ] && [ "$units" = "nhours" ]; then
        JOB_QUEUE="pdebug"
    else
        JOB_QUEUE="pbatch"
    fi  
...
runtime_options() {
    ...
    ./atmchange mac_aero_mic::atm_procs_list=shoc,cld_fraction,spa,p3,nudging
    ...
    ./atmchange physics::mac_aero_mic::nudging::nudging_filenames_patterns=${NUDGING_ROOT}/era5p_TRaave_L128.????????.3h.ncpdq_FillValue.v1.nc
    ...
    #./atmchange physics::mac_aero_mic::nudging::nudging_timescale=172800 #2day
    ./atmchange physics::mac_aero_mic::nudging::nudging_timescale=86400 #1d
    ...
    #./atmchange physics::mac_aero_mic::nudging::nudging_weights_file=/p/vast1/zhang73/grids2/${RRMgrid}/gridprocessing/${RRMgrid}pg2_weighting_file.nc
    ./atmchange physics::mac_aero_mic::nudging::nudging_weights_file=/p/vast1/zhang73/grids2/${RRMgrid}/gridprocessing/${RRMgrid}pg2_weighting_file.v1.nc
    ...
    ./xmlchange SSTICE_DATA_FILENAME="/p/vast1/zhang73/DATA/data_hiccup/sst_ice.daymean.2020_2025_noleap.fillmsg.aave_1x1.fmt-c260502.nc"
    ...
    # manually modify --time in flux command
    IFS=: read -r hh mm ss <<< "$walltime"
    walltime_m=$((10#$hh * 60 + 10#$mm))
    walltime_flux="${walltime_m}m"
    echo 'walltime_flux = '${walltime_flux}
    perl -0pi -e "s|<arg flag=\"--time\" name=\"[^\"]*\"/>|<arg flag=\"--time\" name=\"$walltime_flux\"/>|" env_batch.xml
    ./xmlchange JOB_WALLCLOCK_TIME=${walltime}
    ./xmlchange BATCH_COMMAND_FLAGS="  --time ${walltime_flux} --queue \$JOB_QUEUE --bank \$PROJECT"



# https://github.com/E3SM-Project/eamxx-scripts/blob/master/run_scripts/run.ne256.decadal.20251029.sh
...
cat <<EOF >> 1hA_site-all.yaml
%YAML 1.1
---
averaging_type: average
max_snapshots_per_file: 8760
filename_prefix: 1hA_site-all
horiz_remap_file:  ${DIN_LOC_ROOT}/atm/scream/maps/map_Chilene32x32v1pg2_to_powerplants_demand_substations_hydroplants_station.pg2icol_site-all.20260701.5.nc
fields:
  physics_pg2:
    field_names:
    - RelativeHumidity_at_2m_above_surface
...
cat <<EOF >> 1hA_clbuffer.yaml
%YAML 1.1
---
averaging_type: average
max_snapshots_per_file: 8760
filename_prefix: 1hA_clbuffer
...
cat <<EOF >> 1mA_clbuffer.yaml
%YAML 1.1
---
averaging_type: average
max_snapshots_per_file: 12
filename_prefix: 1mA_clbuffer
horiz_remap_file: \${DIN_LOC_ROOT}/atm/scream/maps/map_Chilene32x32v1pg2_to_Chilebuffer.pg2icol_shp.20260501.5.nc
fields:
  physics_pg2:
    field_names:
...
cat <<EOF >> 1mA_pg2.yaml
%YAML 1.1
---
averaging_type: average
max_snapshots_per_file: 12
filename_prefix: 1mA_pg2
fields:
  physics_pg2:
    field_names:
    - RelativeHumidity_at_2m_above_surface
...
cat <<EOF >> 1mA_nudging.yaml
%YAML 1.1
---
averaging_type: average
max_snapshots_per_file: 12
filename_prefix: 1mA_nudging
fields:
  physics_pg2:
    field_names:
    - U
...
cat <<EOF >> 3hI_nudging_ne30pg2.yaml
%YAML 1.1
---
averaging_type: instant
max_snapshots_per_file: 2920
filename_prefix: 3hI_nudging_ne30pg2
horiz_remap_file: \${DIN_LOC_ROOT}/atm/scream/maps/map_Chilene32x32v1pg2_to_ne30pg2_traave.20250426.nc
fields:
  physics_pg2:
    field_names:
...
#---only for checking in test runs--->
    ...
    ./atmchange output_yaml_files+="./1hA_site-all.yaml"
    ./atmchange output_yaml_files+="./1hA_clbuffer.yaml"
    ./atmchange output_yaml_files+="./1mA_clbuffer.yaml"
    ./atmchange output_yaml_files+="./1mA_pg2.yaml"
    #---below: only for checking in test runs
    ./atmchange output_yaml_files+="./1mA_nudging.yaml"
    ./atmchange output_yaml_files+="./3hI_nudging_ne30pg2.yaml"
